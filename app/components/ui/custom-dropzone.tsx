...
const variants = {
  base: 'relative rounded-md w-full h-full flex justify-center items-center flex-col cursor-pointer min-h-[150px] min-w-[200px] border border-dashed border-gray-400 dark:border-gray-300 transition-colors duration-200 ease-in-out',
  image:
...
const CustomImageDropzone = React.forwardRef<HTMLInputElement, InputProps>(
  (
    { dropzoneOptions, value, className, disabled, onChange, onFilesAdded, customContent },
    ref,
  ) => {
    ...
    return (
      <div>
        <div className={clsx("", {
          "flex w-full": !value || value.length == 0,
          "grid grid-cols-4 grid-rows-2 gap-2 sm:grid-cols-[repeat(2,1fr)] lg:grid-cols-[repeat(3,1fr)] xl:grid-cols-[repeat(4,1fr)]": value && value.length > 0
        })}>
          {/* Images */}
          {value?.map(({ file, progress }, index) => (
            <div
              key={index}
              className={clsx(variants.image + ' aspect-square h-full', {
            ...
            >
              ...
              {/* Remove Image Icon */}
              {imageUrls[index] && !disabled && (progress === 'PENDING' || progress === 100) && (
                <div
          ...
          {(!value || value.length < (dropzoneOptions?.maxFiles ?? 0)) && (
            <div
              {...getRootProps({
                className: dropZoneClassName + (!value || value.length == 0 ? " w-full h-full aspect-auto" : " aspect-square"),
              })}
            >
              <input ref={ref} {...getInputProps()} />
              <div className="flex flex-col items-center justify-center text-xs text-gray-400">
                <UploadCloudIcon className="mb-2 h-7 w-7" />
                <div className="text-gray-400">{customContent ? customContent : "Drag 3-5 images here"}</div>
                <div className="mt-3">
...
CustomImageDropzone.displayName = 'MultiImageDropzone';
